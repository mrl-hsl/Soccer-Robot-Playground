...
int World::updateWindow() {
    while (true) {
        ...
        if (robot.borderCheck() == 0) {
            ...
            if (moveSignal == true) {
                behavior.setMousePoint(point, arrowHead);
                behavior.behaviorVelocity();
...
void World::create() {
    ...
    if(moveSignal == 1) {
        circle(realWorld, point, modelScale / 20, Scalar(200, 0, 200), 2, 8, 0);
        arrowedLine(realWorld,point,arrowHead,Scalar(200, 0, 200),2,8,0);
    }
    ...
    if (robot.borderCheck() == 0) {
        if (mouseFlag == 1) {
            switch(event) {
                ...
                // cout << x << " " << y << endl;
                    if(makeArrow == true) {
                        double arrowSize = sqrt(pow(x - point.x, 2) + pow(y - point.y, 2));
                        if(arrowSize <= 50) {
                            arrowHead.x = x;
                            arrowHead.y = y;
                        }
